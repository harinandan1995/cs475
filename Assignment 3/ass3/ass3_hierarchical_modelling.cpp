...
void load_scene(){
    ...
    for(int i=0;i<num_objects;i++){
      ...
      csX75::HNode* node;
      if(parent_index == -1) node = new csX75::HNode(NULL,v_positions[i].size(),v_positions[i],v_colors[i],v_positions[i].size()*sizeof(glm::vec4),v_colors[i].size()*sizeof(glm::vec4));
      else node = new csX75::HNode(node_list[parent_index],v_positions[i].size(),v_positions[i],v_colors[i],v_positions[i].size()*sizeof(glm::vec4),v_colors[i].size()*sizeof(glm::vec4));
